/*	Filename	:	clinkdelsearch.c
 *	Date		:	04-08-2010
 *	Author		:	Stalin Srinivasan.S
 *	Description	:	to demonstrate about the circular linked list deletion
 */
 ...
 void delstart()
 {
	...
	first=temp;						//deleting the first node of the list
	free(pa);
	return;
 }
 ...
 void delatpos()
 {
	...
	for(pa=first,i=0;i<pos-2;pa=pa->next,i++);
	temp=pa->next;
        pa->next=temp->next;		
	free(temp);
 	return;
 ...
 void search()
 {
	...
	scanf("%d",&val);
	for(temp=first,i=0;temp->next!=first;temp=temp->next,i++)		//search particular element as per previous function defn and display result
	{
		if(temp->data==val)
		{
			c=2;
