/*	Filename	:	linkinsertsort.c
 *	Date		:	03-08-2010
 *	Author		:	Stalin Srinivasan.S
 *	Description	:	to demonstrate about the linked list insertion
 */
 ...
 {
	struct node *temp;
	int i;							// traverse from first to last and display content of nodes on path
 ...
 {
	struct node *temp,*pa;
	for(pa=first;pa->next!=NULL;pa=pa->next);			//inserting node at end of the list
	temp=malloc(sizeof(struct node));
